  ...
  <a href="#" class="card border border-2 border-gray-300 border-hover">
    ...
    <div class="card-header border-0 pt-9">
      ...
      <div class="card-toolbar">
        <span :class="getStatusDataBadgeColor" class="badge fw-bolder me-auto px-4 py-3">{{ getStatus }}</span>
      </div>
      <!-- end::Card toolbar -->
    </div>
    <!-- end:: Card header -->
        ...
        <div class="symbol-group symbol-hover">
          <template v-for="(user, index) in users" :key="index">
            ...
            <div class="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip" :title="user.title">
              ...
              <span v-else class="symbol-label fw-bolder" :class="`bg-${user.state} text-inverse-${user.state}`">{{
                user.initials }}</span>
            </div>
            <!-- begin::User -->
          </template>
