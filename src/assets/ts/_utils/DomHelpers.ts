...
function slide(el: HTMLElement, dir: string, speed: number, callback: any) {
  ...
  if (dir === 'up') {
    ...
    if (calcPaddingTop) {
      ElementAnimateUtil.animate(0, calcPaddingTop, speed, (value: number) => {
        el.style.paddingTop = `${calcPaddingTop - value}px`
      })
    }

    if (calcPaddingBottom) {
      ElementAnimateUtil.animate(0, calcPaddingBottom, speed, (value: number) => {
...
function colorDarken(color: string, amount: number) {
  ...
  return (color = `#${subtractLight()}${subtractLight()}${subtractLight()}`)
}
